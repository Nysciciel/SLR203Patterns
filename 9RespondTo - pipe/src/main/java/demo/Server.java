...
public class Server extends UntypedAbstractActor{
	...
	public void onReceive(Object _message) throws Throwable {
		if(_message instanceof Request){
			...
			log.info("("+getSelf().path().name()+") received a message from ("+ getSender().path().name() +")"+"contains:" + message.s+" "+message.ref );
			Response toSend = new Response(message.s + "validated");
			Thread.sleep(1000);
			message.ref.tell(toSend, getSelf());
		}
	}
